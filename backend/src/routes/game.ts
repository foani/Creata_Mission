...
import { PrismaClient, User, GameLog } from '@prisma/client';
...
interface GameSubmissionRequest extends Request {
  body: {
...
function validateGameSubmission(req: GameSubmissionRequest, res: Response, next: NextFunction) {
  ...
  if (!ethers.isAddress(walletAddress)) {
    ...
    if (!Object.values(GAME_TYPES).includes(gameType as any)) {
    return res.status(400).json({
      success: false,
...
router.post('/submit', validateGameSubmission, async (req: GameSubmissionRequest, res: Response) => {
  try {
  ...
  res.json({
    ...
    data: {
      gameTypes: Object.entries(GAME_TYPES).map(([key, value]) => ({
        ...
        value,
        name: {
                    ko: key === 'BINARY' ? '암호화폐 가격 예측' :
              key === 'DERBY' ? '레이지 더비' : '리버스 다트',
                    en: key === 'BINARY' ? 'CryptoPricePrediction' :
              key === 'DERBY' ? 'Lazy Derby' : 'Reverse Darts',
          vi: key === 'BINARY' ? 'Dự đoán giá tiền mã hóa' :
              key === 'DERBY' ? 'Derby lười' : 'Phi tiêu ngược',
          ja: key === 'BINARY' ? '暗号通貨価格予測' :
